...
//  picpro
//
//  Created by mica dai on 2024/7/19.
//
...
struct UploadImageView: View {
    ...
    private func uploadImage() {
        ...
        #elseif os(macOS)
        guard let tiffData = image.tiffRepresentation, let bitmap = NSBitmapImageRep(data: tiffData), let imageData = bitmap.representation(using: .jpeg, properties: [:]) else {
            uploadStatusMessage = "Failed to convert image to data."
            isUploading = false
            return
        }
